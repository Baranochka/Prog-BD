...
if __name__ == "__main__":
    start_session("sa", "12345678aA!")
    debug(fetch_all_data())
    person = "ahmed"
    debug(fetch_person_by(surname=person))

    id = 2
    new_surname = "АХМЕД"

    update(
        f"UPDATE persons SET {PersonsField.surname} = :new_{PersonsField.surname} WHERE {PersonsField.id} = :{PersonsField.id}",
        {f"new_{PersonsField.surname}": new_surname, f"{PersonsField.id}": id}
        )
